...
export function RaioXResultTabs({ activeView, onChange }: RaioXResultTabsProps) {
  return (
    <div className="mx-auto mb-5 flex max-w-md rounded-2xl border border-entrelinhas-champagne/10 bg-white/[0.035] p-1.5 backdrop-blur">
      {tabs.map((tab) => {
        const Icon = tab.icon;
        ...
        return (
          <button
            ...
            className={`flex min-w-0 flex-1 items-center justify-center gap-1.5 rounded-xl px-2.5 py-3 text-xs font-bold transition sm:text-sm ${
              isActive ? "bg-entrelinhas-gold/90 text-entrelinhas-ink shadow-gold" : "text-entrelinhas-muted hover:bg-white/[0.055] hover:text-white"
            }`}
          >
            <Icon size={15} />
