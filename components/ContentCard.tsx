...
export function ContentCard({ item, generatedCopy, isGenerating, onGenerateCopy }: ContentCardProps) {
  ...
  return (
    <article className="group rounded-lg border border-entrelinhas-line bg-[#151520]/92 p-4 transition hover:border-entrelinhas-gold/70 hover:bg-[#191926]">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <span className="inline-flex items-center gap-1 rounded border border-entrelinhas-gold/35 bg-entrelinhas-gold/10 px-2 py-1 text-entrelinhas-gold">
              <Instagram className="h-3.5 w-3.5" />
              {formatLabel[item.format]}
            </span>
            <span className="rounded border border-entrelinhas-violet/30 bg-entrelinhas-violet/10 px-2 py-1 text-violet-200">
              {item.platform}
            </span>
            <span className="rounded border border-entrelinhas-teal/30 bg-entrelinhas-teal/10 px-2 py-1 text-entrelinhas-teal">
              {item.moment}
            </span>
          </div>
          <h3 className="mt-3 text-lg font-semibold leading-6 text-stone-50">{item.theme}</h3>
        </div>
        <ChevronRight className="mt-1 h-5 w-5 shrink-0 text-stone-500 transition group-hover:translate-x-0.5 group-hover:text-entrelinhas-gold" />
      </div>
      ...
      <div className="mt-4 grid gap-2 text-xs text-stone-300 sm:grid-cols-2">
        <span className="inline-flex items-center gap-2 rounded-md bg-white/[0.035] px-3 py-2">
          <Layers3 className="h-4 w-4 text-entrelinhas-teal" />
          {item.hookType}
        </span>
        <span className="inline-flex items-center gap-2 rounded-md bg-white/[0.035] px-3 py-2">
          <Brain className="h-4 w-4 text-entrelinhas-rose" />
          {item.scienceBase}
        </span>
        <span className="inline-flex items-center gap-2 rounded-md bg-white/[0.035] px-3 py-2">
          <BadgeCheck className="h-4 w-4 text-entrelinhas-gold" />
          {item.objective}
        </span>
        <span className="inline-flex items-center gap-2 rounded-md bg-white/[0.035] px-3 py-2">
          <MousePointerClick className="h-4 w-4 text-entrelinhas-violet" />
          {item.ctaType}
        </span>
      </div>
      ...
      <div className="mt-4 flex items-center gap-2 border-t border-entrelinhas-line pt-3 text-sm text-stone-200">
        <Flame className="h-4 w-4 text-entrelinhas-rose" />
        Intensidade emocional {item.score.emotionalIntensity}/10
      </div>

      {generatedCopy ? (
        <div className="mt-4 rounded-md border border-entrelinhas-teal/25 bg-entrelinhas-teal/10 p-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="text-xs uppercase tracking-[0.18em] text-entrelinhas-teal">Copy gerada</p>
          ...
          <p className="mt-2 text-xs text-entrelinhas-gold">{generatedCopy.copy.hashtags.join(" ")}</p>
        </div>
      ) : null}
      ...
      {onGenerateCopy ? (
        <button
          ...
          className="mt-4 inline-flex h-10 w-full items-center justify-center gap-2 rounded-md border border-entrelinhas-line bg-entrelinhas-night px-3 text-sm font-semibold text-stone-100 transition hover:border-entrelinhas-gold hover:text-white disabled:cursor-not-allowed disabled:opacity-60"
        >
...
function Score({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-md border border-white/5 bg-entrelinhas-void/35 px-2 py-2">
