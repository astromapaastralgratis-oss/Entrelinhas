    ...
    <section className="rounded-lg border border-astral-line bg-astral-panel/72 p-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded border border-astral-teal/30 bg-astral-teal/10 px-3 py-1 text-xs text-astral-teal"
        >
      ...
      {open ? (
        <div className="mt-4 space-y-2">
          {latest.length === 0 ? (
            ...
            latest.map((log) => (
              <div key={log.id} className="rounded-md border border-white/5 bg-astral-void/35 p-3">
                <p className={log.level === "error" ? "text-sm font-semibold text-astral-rose" : "text-sm font-semibold text-stone-100"}>
                  {log.message}
                </p>
