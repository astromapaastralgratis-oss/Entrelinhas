...
describe("text generation service", () => {
  it("builds a compact provider-agnostic prompt without long branding", () => {
    ...
    expect(prompt).not.toContain("Você é o motor de copy do Entrelinhas");
  });

  it("parses fenced JSON returned by providers", () => {
    const copy = parseAiJson(`\`\`\`json
{"title":"Energia da semana","subtitle":"Um sinal simples para hoje","slides":[],"caption":"Legenda com CTA.","hashtags":["#EntrelinhasPessoal","#Autoconhecimento","#Astrologia","#ClarezaEmocional","#EnergiaDoDia"],"cta":"Comente sua palavra.","pinnedComment":"Qual parte fez sentido?","qualityNotes":{"scrollStop":"gancho","identification":"tema","action":"cta"}}
\`\`\``);
